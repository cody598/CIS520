               ...
               thread_func *function, void *aux) 
{
  ...
  /* Set t's parent id to the current thread id */
...
*thread_get_by_id (tid_t tid)
{
  ...
  elem = list_tail (&all_list);
  while ((elem = list_prev (elem)) != list_head (&all_list))
    {
      thread = list_entry (elem, struct thread, allelem);
      if (thread->tid == tid && thread->status != THREAD_DYING)
        return thread;
    }
  return NULL;
}